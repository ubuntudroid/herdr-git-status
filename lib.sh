...
gci_latest_ci() {
  local repo="$1" url parsed enc resp run st cc up
  ...
  if [ "$GCI_PROVIDER" = "gitlab" ]; then
    ...
    # Fork PRs' `pull_request` workflow runs execute in the BASE repo (?branch= matches
    # head_branch there); on no-hit, retry the upstream slug (retry errors = "no run").
    if [ -z "$run" ] && up="$(gci_upstream_path "$repo")" && [ -n "$up" ] && [ "$up" != "$GCI_PATH" ]; then
      resp="$(cd "$repo" && gh api "repos/$up/actions/runs?branch=$GCI_BRANCH&per_page=1" 2>/dev/null)" \
        && run="$(printf '%s' "$resp" | jq -c '.workflow_runs[0] // empty' 2>/dev/null)" \
        && [ -n "$run" ] && GCI_CI_PATH="$up" || true
    fi
    [ -n "$run" ] || return 0
...
gci_review_for_mr() {
  ...
  if [ "$provider" = "gitlab" ]; then
    ...
    [ -n "$dms" ] || return 0
    # NB: jq's // swallows false, so `.field // true` would invert an explicit false.
    blocking="$(printf '%s' "$resp" | jq -r '(.blocking_discussions_resolved != false) | tostring' 2>/dev/null)"
    ...
    # Presence check on the PR object, not on isDraft — jq's // swallows false, so
    # `.isDraft // empty` would bail on every non-draft PR.
    draft="$(printf '%s' "$resp" | jq -r '.data.repository.pullRequest | if . == null then empty else (.isDraft | tostring) end' 2>/dev/null)"
    [ -n "$draft" ] || return 0
