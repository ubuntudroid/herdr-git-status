...
- `GITLAB_CI_ICON_CONFLICT` / `_CHANGES` / `_APPROVED` / `_DRAFT` / `_AWAITING` — review-state
  glyphs (defaults `⚠️` `💬` `✅` `📝` `👀`). The sidebar badge only ever shows
  conflict/changes/approved; draft/awaiting appear in the My MRs pane.
